...
template <typename E, bool is_lite,
          ...
          typename = std::enable_if_t<is_proto_enum<E>::value>,
          typename = std::enable_if_t<is_lite == LiteEnumFuncs<E>::kIsDefined>>
using EnableIfProtoEnum = int;
...
std::string AbslUnparseFlag(Enum e) {
  ...
  return name.empty() ? absl::StrCat(e) : std::string(name);
}
}  // namespace generated_enum
